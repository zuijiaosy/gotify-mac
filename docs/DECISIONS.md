...
仅提供「显示/隐藏主界面」「全部已读」两个可自定义全局快捷键，默认 Control+Option+G / Control+Option+R，可清除。使用 Carbon `RegisterEventHotKey` 独占注册，不引入第三方依赖，不申请键盘监听权限。注册器由应用生命周期持有，按下/释放防重复；设置保存与注册失败必须显示错误并回退原绑定。

快捷键存入现有 config.json，缺字段采用默认，显式 null 保留用户清除操作，畸形单项独立禁用。新增 `AppModel.saveShortcuts` 作为协调者内的写入入口：先落盘，再更新内存，不重连服务器。此条补充 ADR-009 的单一 saveConfig 方法约束，配置写入仍统一由 AppModel 管理。

SwiftUI `MenuBarExtra` 没有公开的面板显示控制接口，因此使用 AppKit 生命周期、`NSStatusItem` 与 transient `NSPopover` 承载原 SwiftUI 内容。设置窗口改由同一控制器持有 `NSWindow` + SwiftUI TabView。不查找私有窗口、不模拟点击菜单栏、不依赖辅助功能。

### Consequences

- 保留无 Dock 图标、未读角标、单栏/双栏硬切、点击外部关闭与关闭后回单栏；原 MenuBarExtra 窗口 frame 修补移除，尺寸改由 popover 控制。
- 全部已读复用当前水位线逻辑，无未读时不写配置、不打开面板、不抢焦点；仍仅在本地生效。
- 录入期间暂停本应用快捷键；Escape、切换标签、窗口失焦或关闭会取消录入并恢复注册。
- 快捷键按物理键位保存并以 ANSI 名称展示。系统独占注册冲突可以检测；系统保留键和其他软件的非独占键盘监听无法保证全部检测。
- AppKit 生命周期接管后显式提供设置、退出与标准编辑菜单，保证输入框复制/粘贴功能。
- 原生 UI 行为需桌面会话验证；单测使用虚构消息、临时配置和模拟注册器，避免触碰真实消息和用户快捷键。
