...
- **面板白底优化（2026-09-05）**：消息面板改为不透明白底，固定浅色主题与深色文字，避免半透明底色影响阅读。原生 UI 冒烟检查增加深色应用外观下的原始像素检查，已验证单栏、双栏背景为不透明白色。**v0.3.1 已发布**：提交 `ca0388b`，Release workflow 测试、双架构构建与 DMG 发布全部成功，产物 `Gotify-Mac-0.3.1.dmg`。
- **v0.3.0 已发布（2026-09-05）**：提交 `f1db953`，Release workflow 成功，包含 Apple Silicon / Intel 通用 DMG。

- **两个全局快捷键（2026-09-05，ADR-015）**：默认 Control+Option+G 显示/隐藏消息面板，Control+Option+R 后台全部已读；「设置 → 通用」仅增加这两项，支持录入、清除、立即保存、冲突提示与失败回退。Carbon 独占注册，不申请辅助功能或输入监控权限；录入期间暂停注册，取消、失焦、关闭或切换标签后恢复，按下/释放去除长按重复。配置兼容旧文件，null 表示用户禁用，错误快捷键字段不影响其他配置。菜单栏改由 NSStatusItem + transient NSPopover 承载原 SwiftUI 面板，鼠标与快捷键使用同一入口；设置改为控制器持有的 NSWindow + SwiftUI TabView，保留顶部标签与标准编辑菜单。全部已读在无未读时为空操作，避免空列表将持久化水位线清零。
  验证：95 项单元测试通过（新增 13 项），Release 构建通过；`bash scripts/shortcut-ui-smoke.sh` 使用虚构消息和临时配置验证单栏/双栏尺寸、鼠标与快捷键动作交替、关闭重置、后台全部已读落盘、设置窗口和录入暂停/恢复，截图位于忽略目录 `build/shortcut-ui/`。真实跨应用键盘输入、点击面板外部关闭仍待手工验收；自动化进程未获辅助功能权限。未替换已安装应用，未发布。
...
- 快捷键原生补充验证已通过：Carbon 注册、真实 C 回调分发、长按去重及注销后重新注册；该检查发送进程内 Carbon 事件，不模拟跨应用物理按键。

## In Progress

- 全局快捷键实机验收：启动新构建，在其他应用前台时测试显示/隐藏与后台全部已读；检查点击面板外部关闭、修改组合键及重启后恢复、冲突提示。功能不需要辅助功能权限，只有自动模拟系统键盘输入的测试需要。
...
4. 后续迭代：睡眠/唤醒处理。v0.3.1 已发布，安装包由标签触发的 Release workflow 生成；本机已安装应用未自动替换。
