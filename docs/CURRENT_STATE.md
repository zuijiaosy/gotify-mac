...
- **面板白底优化（2026-09-05）**：消息面板改为不透明白底，固定浅色主题与深色文字，避免半透明底色影响阅读。原生 UI 冒烟检查增加深色应用外观下的原始像素检查，已验证单栏、双栏背景为不透明白色。准备作为 v0.3.1 发布。
...
4. 后续迭代：睡眠/唤醒处理。v0.3.0 已发布；白底优化准备发布为 v0.3.1，安装包由标签触发的 Release workflow 生成。
